...
const texture = new THREE.TextureLoader().load(
  "/textures/jong-marshes-unsplash.jpg"
);
const planeMaterial = new THREE.MeshPhongMaterial({
  map: texture,
  side: THREE.DoubleSide,
...
function animate() {
  ...
  for (let i = 0; i < array.length; i += 3) {
    // x
    array[i] = originalPosition[i] + Math.cos(frame + randomValues[i]) * 0.1;
    //y
    array[i + 1] =
      originalPosition[i + 1] + Math.sin(frame + randomValues[i + 1]) * 0.1;
  }
